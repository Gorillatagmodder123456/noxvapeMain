local NoxLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/Gorillatagmodder123456/Noxvape/refs/heads/main/noxvape.lua"))()
...
local function isEnemyPlayer(t)
	...
	if player.Team and t.Team and player.Team == t.Team then
		return false
	end
	return true
end
...
local function collectAllEntities()
	local now = os.clock()
	if now - _entityCache.time < ENTITY_TTL then
		return _entityCache.data
	end
	local list = {}
	local myChar = player.Character
	local seen = {}
	local function check(m)
		if seen[m] or m == myChar or Players:GetPlayerFromCharacter(m) then
			return
		end
		seen[m] = true
		local h = m:FindFirstChildOfClass("Humanoid")
		if not h or not isHumanoidAlive(h) then
			return
		end
	...
	recurse(workspace, 0)
	_entityCache.data = list
	_entityCache.time = now
	return list
end
local function getClosestTarget(includeEntities)
	...
	for _, t in ipairs(Players:GetPlayers()) do
		if t ~= player and t.Character and isEnemyPlayer(t) then
			local h = getCharacterHumanoid(t.Character)
			local tr = getCharacterRoot(t.Character)
...
local function findBed()
	if _bedCache and _bedCache.Parent then
		return _bedCache
	end
	local function search(c, d)
		...
		for _, x in ipairs(c:GetChildren()) do
			if x:IsA("BasePart") and string.lower(x.Name) == "bed" then
				return x
			end
	...
	_bedCache = search(workspace, 0)
	return _bedCache
end
NoxLib.addCategory("Combat")
NoxLib.addCategory("Movement")
NoxLib.addCategory("Exploits")
NoxLib.addCategory("Kits")
...
return {
    ...
    getNumber = getNumber,
    getBool = getBool,
    getSwordWeapon = getSwordWeapon,
    getCharacterRoot = getCharacterRoot,
    getCharacterHumanoid = getCharacterHumanoid,
    isHumanoidAlive = isHumanoidAlive,
    isEnemyPlayer = isEnemyPlayer,
    getEntityRoot = getEntityRoot,
    getKnitController = getKnitController,
